...
$sub_folder = 'sites/it262/winter_2023/'; //If app installed in subfolder, place here.  name of folder, no leading or trailing forward or backslash
...
if (!isset($error_reporting)) {
	$config->error_reporting = $default_error_reporting;
} else {
...
define('SUPPORT_EMAIL', 'brandon.davila@seattlecolleges.edu'); # Email of site support
...
if (startSession() && isset($_SESSION['AdminID'])) { #add admin logged in info to sidebar
...
if (startSession() && isset($_SESSION['AdminID']) && $config->theme != 'Bootswatch') {
...
function myErrorHandler($e_number, $e_message, $e_file, $e_line, $e_vars)
{
	...
	$counter++;
	if (LOG_ALL_ERRORS) { # Copy all error info to error log file
		try {
			$errInfo = "[" . date('M-d-Y G:i:s') . "] $e_message in $e_file on line $e_line" . "\n";
			fileWrite(LOG_PATH . $errFile, 'a+', $errInfo);

			if (substr(sprintf('%o', fileperms(LOG_PATH . $errFile)), -4) != '0700') { //check file permission - if NOT set to 0700, fix it!
	...
	if (SHOW_ALL_ERRORS || isset($_SESSION['AdminID'])) { # Display generic error message, with support email from config file
